...
public class MySingleLinkedList<T> { // 단방향 링크드리스트, 반대로 각각의 노드가 자기 이전,다음 노드의 주소를 동시에 가지는 구조의 연결리스트는 더블링크드리스트,
	...
	private Node<T> head; // 첫번째 노드의 주소, T타입
	private int size;
	...
	//inner class 클래스 안에 클래슬 넣는 것
	//연결리스트를 만드려면 노드 클래스가 필요하다. 클래스안에 클래스를 만들어 노드를 외부에서 사용하고 수정하지 못하게 만들기.
	private static class Node<T> { //임의의 가상 타입 T
	...
	public Iterator<T> iterator() //마이 싱글리스트 클래스는 iterator라는 public 메소드를 제공을 한다
	{
		return null;
	}
	private void addFirst( T item ){ //새로운 노드 데이터를 연결리스트 맨 앞에 추가하는 메소드 
	...
	private void addAfter( Node<T> before, T item){ // 한 노드의 주소를 받아서 before 가리키는 노드 뒤에 데이터를 끼워넣기
	...
	private T removeFirst(){ //연결리스트의 첫번째 노드를 삭제한다. remove된 데이터가 가지고 있는 타입을 리턴 타입으로 지정해줘야 하기 때문에 T타입으로 생성
		if (head == null)
			return null; 
	...
	private T removeAfter( Node<T> before ){ //before가 가리키는 노드의 다음 노드를 삭제한다. 
		if ( before.next == null)
			return null;
